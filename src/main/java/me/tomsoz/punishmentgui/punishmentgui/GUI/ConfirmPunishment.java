...
public class ConfirmPunishment {
    ...
    public Inventory getInventory() {
        String publicicy;
        FileConfiguration cfg = this.plugin.getConfigManager().getGUIConfirm();
        ...
        String guiName = Utils.chat(cfg.getString("gui.name").replaceAll("%punishment%", this.type.toString()).replaceAll("%target%", this.target.getName())).replaceAll("%executor%", this.executor.getName()).replaceAll("%time%", this.time).replaceAll("%publicity%", publicicy).replaceAll("%reason%", this.reason);
        ...
        for (String key : cfg.getConfigurationSection("items").getKeys(false)) {
            ...
            if (mat != null) {
                ...
                String name = Utils.chat(cfg.getString("items." + key + ".name").replaceAll("%punishment%", this.type.toString()).replaceAll("%target%", this.target.getName())).replaceAll("%executor%", this.executor.getName()).replaceAll("%time%", this.time).replaceAll("%publicity%", publicicy);
                List<String> cfglore = cfg.getStringList("items." + key + ".lore");
                List<String> lore = new ArrayList<>();
                for (String s : cfglore)
                    lore.add(Utils.chat(s
                            .replaceAll("%punishment%", this.type.toString())
                            .replaceAll("%target%", this.target.getName()))
                            .replaceAll("%executor%", this.executor.getName())
                            .replaceAll("%time%", this.time)
                            .replaceAll("%publicity%", publicicy)
                            .replaceAll("%reason%", this.reason));
                ...
                if (this.executor.hasPermission(perm)) {
                    meta.setLore(lore);
                } else {
