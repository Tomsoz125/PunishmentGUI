...
public class UnmutePlayer {
    ...
    private void unmutePlayer() {
        ...
        if (this.silent) {
            commands = this.plugin.getConfigManager().getConfig().getStringList("commands.silent.unmute");
        } else {
            commands = this.plugin.getConfigManager().getConfig().getStringList("commands.public.unmute");
        }
        ...
        this.executor.sendMessage(Utils.chat("&aYou've sucessfully unmuted " + this.target.getName()));
    }
}
